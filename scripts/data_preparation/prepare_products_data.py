...
def clean_dataframe(df: pd.DataFrame, stock_quantity_limits: tuple = (2, 8)) -> pd.DataFrame:
    """
    Cleans the DataFrame by removing duplicates and filtering out products based on StockQuantity.
    """
    df = df.drop_duplicates()
    logger.info("Removed duplicates.")
    df = df[(df['StockQuantity'] >= stock_quantity_limits[0]) & (df['StockQuantity'] <= stock_quantity_limits[1])]
    logger.info("Filtered products based on StockQuantity.")
    return df
...
if __name__ == "__main__":
    ...
    input_filepath = "data/raw/products_data.csv"
    output_filepath = "data/prepared/products_data_prepared.csv"
    expected_columns = {
        "ProductID": "int64",
        "ProductName": "object",
        "Category": "object",
